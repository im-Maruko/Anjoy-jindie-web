...
import { Card, Avatar, Typography, Button, Row, Col, Modal, Upload, Tooltip, Space, Empty, } from 'antd';
...
export default function TestPage() {
    ...
    const expenseColumns = [
        // 🌟 优化点1：加上了 fixed: 'left'，让序号永远固定在第一列
        { title: '序号', valueType: 'indexBorder', width: 48, fixed: 'left' },
        {
            ...
            fieldProps: getSearchFieldProps('费用项目')
        },
        { title: '原币申请金额', dataIndex: 'originalAmount', valueType: 'money', width: 140, formItemProps: { rules: [{ required: true, message: '必填' }] }, fieldProps: { style: { width: '100%' } } },
        { title: '可用余额', dataIndex: 'availableBalance', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
        { title: '本位币可用余额', dataIndex: 'baseAvailableBalance', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
        { title: '本位币已用金额', dataIndex: 'baseUsedAmount', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
        {
            title: '费用客户',
            dataIndex: 'expenseCustomer',
            valueType: 'text',
            width: 160,
            fieldProps: getSearchFieldProps('费用客户')
        },
        { title: '呈送单号', dataIndex: 'submissionNo', valueType: 'text', width: 140 },
        { title: '费用说明', dataIndex: 'expenseDesc', valueType: 'text', width: 180, formItemProps: { rules: [{ required: true, message: '必填' }] } },
        { title: '归属日期', dataIndex: 'attributionDate', valueType: 'date', width: 140, formItemProps: { rules: [{ required: true, message: '必填' }] } },
        { title: '下游单据编码', dataIndex: 'downstreamCode', valueType: 'text', width: 150 },
        { title: '多项目模式', dataIndex: 'multiProjectMode', valueType: 'switch', width: 100 },
        {
            title: '项目号A',
            dataIndex: 'projectNoA',
            valueType: 'text',
            width: 150,
            fieldProps: getSearchFieldProps('项目号A')
        },
        {
            title: '预算项目',
            ...
            valueType: 'text',
            width: 150,
            fieldProps: getSearchFieldProps('预算项目')
        },
        {
            title: '资金来源类型',
            dataIndex: 'fundSourceType',
            valueType: 'text',
            width: 150,
            fieldProps: getSearchFieldProps('资金来源类型')
        },
        { title: '备注', dataIndex: 'remark', valueType: 'text', width: 160 },
        { title: '未申请金额原币', dataIndex: 'unappliedOriginal', valueType: 'money', width: 140, readonly: true, fieldProps: { style: { width: '100%' } } },
        { title: '未申请金额本位币', dataIndex: 'unappliedBase', valueType: 'money', width: 140, readonly: true, fieldProps: { style: { width: '100%' } } },
        { title: '已申请金额原币', dataIndex: 'appliedOriginal', valueType: 'money', width: 140, readonly: true, fieldProps: { style: { width: '100%' } } },
        { title: '已申请金额本位币', dataIndex: 'appliedBase', valueType: 'money', width: 140, readonly: true, fieldProps: { style: { width: '100%' } } },
        { title: '是否新品支持', dataIndex: 'isNewProductSupport', valueType: 'switch', width: 120 },

        // 🌟 优化点2：补上漏掉的操作列，并加上 fixed: 'right' 防止被滚动条遮挡
        {
            title: '操作',
            valueType: 'option',
            width: 180,
    ...
    const getDynamicColumns = (type) => {
        ...
        if (type === 'supermarket_approval') {
            return [
                { title: '费用类型', dataIndex: 'expenseType', valueType: 'select', width: 140 },
                { title: '分类', dataIndex: 'category', valueType: 'select', width: 140 },
                { title: '门店名称', dataIndex: 'storeName', valueType: 'text', width: 180 },
                { title: '现有专职', dataIndex: 'fullTimeCount', valueType: 'text', width: 120, fieldProps: { style: { width: '100%' } } },
                { title: '上年度费用金额', dataIndex: 'lastYearExpense', valueType: 'money', width: 160, fieldProps: { style: { width: '100%' } } },
                {
                    title: '上年度年费比',
                ...
                { title: '本年度已使用金额', dataIndex: 'thisYearUsed', valueType: 'money', width: 160, fieldProps: { style: { width: '100%' } } },
                { title: '申请金额', dataIndex: 'applyAmount', valueType: 'money', width: 160, fieldProps: { style: { width: '100%' } } },
                { title: '开始时间', dataIndex: 'startDate', valueType: 'date', width: 140 },
                { title: '结束时间', dataIndex: 'endDate', valueType: 'date', width: 140 },
                { title: '备注', dataIndex: 'remark', valueType: 'text', width: 260 },
        ...
        if (type === 'new_product_entry') {
            return [
                ...
                { title: '规格型号', dataIndex: 'specification', valueType: 'text', width: 140 },
                { title: '单价', dataIndex: 'unitPrice', valueType: 'money', width: 110, fieldProps: { style: { width: '100%' } } },
                { title: '箱价', dataIndex: 'boxPrice', valueType: 'money', width: 110, fieldProps: { style: { width: '100%' } } },
                { title: '进场费用承担情况', dataIndex: 'feeBearingStatus', valueType: 'text', width: 180 },
                { title: '其他', dataIndex: 'otherRemark', valueType: 'text', width: 140 },
                { title: '价格生效日期', dataIndex: 'effectiveDate', valueType: 'date', width: 140 },
                { title: '售价', dataIndex: 'sellingPrice', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
        ...
        if (type === 'ad_expense') {
            return [
                { title: '项目名称', dataIndex: 'projectName', valueType: 'text', width: 180,variant: 'filled' },
                { title: '材质', dataIndex: 'material', valueType: 'text', width: 160 },
                // 拆分长宽
                { title: '长(m)', dataIndex: 'length', valueType: 'digit', width: 100 },
                { title: '宽(m)', dataIndex: 'width', valueType: 'digit', width: 100 },
                // 面积带尾部单位
                { title: '面积', dataIndex: 'area', valueType: 'digit', width: 140, fieldProps: { addonAfter: '㎡', style: { width: '100%' } } },
                // 金额类
                { title: '单价', dataIndex: 'unitPrice', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
                { title: '合计', dataIndex: 'totalAmount', valueType: 'money', width: 140, fieldProps: { style: { width: '100%' } } },
        ...
        if (type === 'ad_billboard') {
            return [
                { title: '项目名称', dataIndex: 'projectName', valueType: 'text', width: 180 },
                { title: '材质', dataIndex: 'material', valueType: 'text', width: 160 },
                // 拆分长宽
                { title: '长(m)', dataIndex: 'length', valueType: 'digit', width: 100 },
                { title: '宽(m)', dataIndex: 'width', valueType: 'digit', width: 100 },
            ...
            >
                ...
                <Card title="基本信息" bordered={false} style={{ marginBottom: 16, borderRadius: 8 }}>
                    ...
                    <Row gutter={[64, 8]}>
                        ...
                        <Col flex="40%"><ProFormText fieldProps={{ variant: 'filled' }} name="reason" label="事由" placeholder="请输入事由" rules={[{ required: true }]} /></Col>
                        ...
                        <Col flex="20%"><ProFormSelect fieldProps={{ variant: 'filled' }} name="payMethod" label="支付方式" placeholder="请选择" options={[{ label: '电汇', value: 'wire' }]} rules={[{ required: true }]} /></Col>
                ...
                {/* 🌟 新增的：费用明细 卡片 */}
                {/* ===================================================================== */}
                <Card title="费用明细" bordered={false} style={{ marginBottom: 16, borderRadius: 8 }}>
                    <EditableProTable
                        rowKey="id"
